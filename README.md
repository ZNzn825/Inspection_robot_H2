...
│                                                                    │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────────┐  │
│  │  AGV 控制 │  │ 机械臂控制│  │ 传感器监测│  │  视频检测显示     │  │
│  │  CAN Bus  │  │  TCP/IP  │  │  串口    │  │  ZED SDK / HSV  │  │
│  └─────┬─────┘  └────┬─────┘  └────┬─────┘  └───────┬──────────┘  │
│        │             │             │                 │             │
├────────┼─────────────┼─────────────┼─────────────────┼─────────────┤
│        │             │             │                 │             │
│  ┌─────┴──────┐ ┌────┴────────┐ ┌──┴───────────┐ ┌──┴──────────┐  │
│  │ IICT6 底盘  │ │ AUBO i16   │ │ H₂传感器     │ │ ZED 2 相机  │  │
│  │ 防爆底盘   │ │ 6-DOF 协作 │ │ + 火焰传感器 │ │ 双目深度    │  │
│  └────────────┘ └────────────┘ └──────────────┘ └─────────────┘  │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                   ROS 导航栈 (QProcess)                       │ │
│  │                                                              │ │
│  │  Livox MID360 ──→ Fast-LIO ──→ 重定位 ──→ move_base ──→ AGV │ │
│  │   (雷达驱动)     (SLAM建图)   (定位)     (路径规划)   (执行)  │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                     UTi586A 红外热像仪                        │ │
...
| 主界面 | 传感器界面 |
|:---:|:---:|
| 待补充 | ![氢气传感器](docs/images/ui-h2-sensor.png) |

| 火焰传感器 | 红外成像 |
|:---:|:---:|
| ![火焰传感器](docs/images/ui-flame-sensor.png) | ![红外](docs/images/ui-infrared.png) |
